...
## 二、 经典 PreNorm 残差退化（PreNorm Dilution）数学推导

要理解 AttnRes 和 mHC 的演进动机，首先需要从数学上证明为什么传统 Transformer 中采用的 **PreNorm 残差连接** 会在高深度的网络下发生“稀释/稀释效应”。

设输入嵌入为 $h_0 \in \mathbb{R}^d$ 。在 PreNorm 架构下，第 $l$ 个子层（可以是 Attention 或 FFN/MoE）的计算公式为：
...
### 1. 隐藏状态的模长增长分析
在初始化或训练早期，假设各个子层输出 $y_l$ 在空间上是大致正交且同分布的，其方差为 $\sigma^2$ 。根据独立随机变量的和的方差性质：

$$
\text{Var}(h_L) = \text{Var}(h_0) + \sum_{l=1}^L \text{Var}(y_l) = \text{Var}(h_0) + L\sigma^2
$$

因此，隐藏状态 $h_L$ 的二阶矩（或欧氏模长）将随层数呈平方根级增长：

$$
\|h_L\|_2 \approx O(\sqrt{L})
$$

若层与层之间的输出由于训练协同而产生正相关（这也是深度神经网络收敛的必然结果），在极端情况下其模长甚至会呈线性增长：
...
### 2. 后续层贡献稀释的数学证明
由于每个子层在计算前必须经过归一化（如 LayerNorm 或 RMSNorm）：
...
由于归一化操作使得输入始终具有单位方差，故在合理的网络初始化下，子层输出的期望模长是有界的：

$$
\mathbb{E}[\|y_l\|_2] \approx C_0
$$

现在我们分析第 $l$ 层的输出 $y_l$ 对最终表示 $h_L$ （以及后续层输入）的**实际相对贡献率** $\gamma_l$ ：
...
**结论**：
随着模型层数 $L$ 的增加，后序层在累加时所面对的基数 $h_{l-1}$ 的模长变得极大，从而导致当前层通过归一化后的有效更新幅度被压制为：

$$
\lim_{L \to \infty} \gamma_l = 0
$$

这意味着，**在极深的网络中，越往后的层对残差主干的实际特征改变能力就越弱**。前序的累加状态如同一股巨大的洪流，后续新生成的微弱特征信号很难改变洪流的方向，此即 **PreNorm 稀释问题（PreNorm Dilution）**。
