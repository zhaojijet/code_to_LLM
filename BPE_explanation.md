...
设我们在某步要合并 pair $P^{\ast} = (u, v)$ 为新 Token $w$ 。
通过 `pair_positions[P^{\ast}]`，我们得到了所有包含该 pair 的节点指针集合。对于每一个位置指针 `pos`（指向包含元素 $u$ 且其后继为 $v$ 的链表节点）：
...
1. **Pop 操作**：每次迭代从堆中 Pop 出当前频数最高的 pair，复杂度为 $O(\log U)$ 。
2. **位置局部合并**：设被 Pop 出来的 pair $P^{\ast}$ 的出现次数为 $k$ 。我们需要对这 $k$ 个位置进行局部合并：
   - 对于每个位置，修改双向链表指针的复杂度为 $O(1)$ 。
   - 对于每个位置，需要对邻近的 $P_L, P_R, P'_L, P'_R$ 共 4 个 pair 的频数和位置表进行局部更新。
   - 每次在最大堆中更新 pair 频数的复杂度为 $O(\log U)$ ，因此单个位置的更新开销为 $O(\log U)$ ，这 $k$ 个位置的总开销为 $O(k \log U)$ 。
3. **节点合并总量上限**：由于每次合并操作都使得语料库中的 Token 总数减少了 1，在整个算法运行期间，所有被合并掉的节点总数（即所有迭代中 $k$ 的累加值）最大不会超过 $N$ ：
