...
#### 案例展示：Attention Residuals 的计算步骤

设模型维度为 $d = 2$ ，我们处在第 $l = 3$ 层。此时模型需要聚合前序 $3$ 个历史状态：
* $v_0$ (Embedding) = $[1.0, 0.0]^T$
* $v_1$ (Layer 1 output) = $[0.0, 1.0]^T$
* $v_2$ (Layer 2 output) = $[1.0, 1.0]^T$

第 $3$ 层专属的可学习伪查询向量为 $q_3 = [0.8, -0.2]^T$ 。

**第一步：对 Key 进行归一化计算 $k_i = \text{RMSNorm}(v_i)$**
由于 $\text{RMS}(v) = \sqrt{\frac{1}{d} \sum_j v_j^2}$ ，我们计算各层 RMS：
* $\text{RMS}(v_0) = \sqrt{\frac{1^2 + 0^2}{2}} \approx 0.707 \implies k_0 = \frac{[1.0, 0.0]^T}{0.707} \approx [1.414, 0.0]^T$
* $\text{RMS}(v_1) = \sqrt{\frac{0^2 + 1^2}{2}} \approx 0.707 \implies k_1 = \frac{[0.0, 1.0]^T}{0.707} \approx [0.0, 1.414]^T$
* $\text{RMS}(v_2) = \sqrt{\frac{1^2 + 1^2}{2}} = 1.0 \implies k_2 = \frac{[1.0, 1.0]^T}{1.0} = [1.0, 1.0]^T$

**第二步：计算注意力得分 $e_{3, i} = \frac{q_3^T k_i}{\sqrt{d}}$**
这里使用缩放因子 $\sqrt{d} = \sqrt{2} \approx 1.414$ ：
* $e_{3, 0} = \frac{[0.8, -0.2] \cdot [1.414, 0.0]^T}{1.414} = 0.8$
* $e_{3, 1} = \frac{[0.8, -0.2] \cdot [0.0, 1.414]^T}{1.414} = -0.2$
* $e_{3, 2} = \frac{[0.8, -0.2] \cdot [1.0, 1.0]^T}{1.414} \approx \frac{0.6}{1.414} \approx 0.424$

**第三步：计算注意力权重 $\alpha_{3, i} = \text{softmax}(e_{3, i})$**
* $\exp(e_{3, 0}) = \exp(0.8) \approx 2.226$
* $\exp(e_{3, 1}) = \exp(-0.2) \approx 0.819$
* $\exp(e_{3, 2}) = \exp(0.424) \approx 1.528$
* 分母和 $\sum \exp(e) = 2.226 + 0.819 + 1.528 = 4.573$
* $\alpha_{3, 0} = \frac{2.226}{4.573} \approx 0.487$
* $\alpha_{3, 1} = \frac{0.819}{4.573} \approx 0.179$
* $\alpha_{3, 2} = \frac{1.528}{4.573} \approx 0.334$

**第四步：加权聚合输出得到第 $3$ 层的输入 $x_3$**
* $x_3 = \alpha_{3,0} \cdot v_0 + \alpha_{3,1} \cdot v_1 + \alpha_{3,2} \cdot v_2$
* $x_3 \approx 0.487 \cdot [1.0, 0.0]^T + 0.179 \cdot [0.0, 1.0]^T + 0.334 \cdot [1.0, 1.0]^T = [0.821, 0.513]^T$
...
#### 案例展示：Hyper-Connections 的多流混合与不稳定性

设通道数（Stream数）为 $C = 2$ ，特征维度 $d = 2$ 。在第 $l$ 层，多流状态矩阵 $X_l \in \mathbb{R}^{2 \times 2}$ 为：

$$
X_l = \begin{bmatrix} x_{l, 1} \\ x_{l, 2} \end{bmatrix} = \begin{bmatrix} 1.0 & 2.0 \\ -1.0 & 0.0 \end{bmatrix}
$$

其两行代表两个平行通道。此时两行向量的欧氏模长平方之和（代表信号总能量）为：

$$
\|X_l\|_F^2 = 1.0^2 + 2.0^2 + (-1.0)^2 + 0.0^2 = 6.0
$$

若网络学到的无约束混合矩阵为：

$$
H_l = \begin{bmatrix} 1.5 & -0.5 \\ 0.5 & 1.2 \end{bmatrix}
$$

其最大奇异值（谱范数）为 $\|H_l\|_2 \approx 1.63 > 1$ 。进行混合计算 $X'_l = H_l X_l$ ：

$$
X'_l = \begin{bmatrix} 1.5 & -0.5 \\ 0.5 & 1.2 \end{bmatrix} \begin{bmatrix} 1.0 & 2.0 \\ -1.0 & 0.0 \end{bmatrix} = \begin{bmatrix} 2.0 & 3.0 \\ -0.7 & 1.0 \end{bmatrix}
$$

计算混合后两流状态的模长平方和：

$$
\|X'_l\|_F^2 = 2.0^2 + 3.0^2 + (-0.7)^2 + 1.0^2 = 14.49
$$

**结论**：可以看出，仅进行了一层混合操作，总信号能量就从 $6.0$ 膨胀到了 $14.49$ （约 $2.4$ 倍）。由于 $H_l$ 的最大奇异值大于 1，多层累积后信号的模长将以指数速度呈爆炸式增长。
...
#### 案例展示：Manifold-Constrained Hyper-Connections (mHC) 的双随机约束与混合

使用上文相同的参数。为了防止信号爆炸，mHC 需将原始无约束参数矩阵 $W_l = \begin{bmatrix} 1.5 & -0.5 \\ 0.5 & 1.2 \end{bmatrix}$ 投影至 Birkhoff 多面体。

**第一步：使用 Exp 函数实现非负初始化**

$$
A^{(0)} = \exp(W_l) = \begin{bmatrix} e^{1.5} & e^{-0.5} \\ e^{0.5} & e^{1.2} \end{bmatrix} \approx \begin{bmatrix} 4.482 & 0.607 \\ 1.649 & 3.320 \end{bmatrix}
$$

**第二步：进行第 1 轮 Sinkhorn-Knopp 迭代（行列交替归一化）**
1. 行归一化：
   * 行 1 和 = $4.482 + 0.607 = 5.089 \implies [4.482/5.089, 0.607/5.089] \approx [0.881, 0.119]$
   * 行 2 和 = $1.649 + 3.320 = 4.969 \implies [1.649/4.969, 3.320/4.969] \approx [0.332, 0.668]$
   得到 $A^{(1)} = \begin{bmatrix} 0.881 & 0.119 \\ 0.332 & 0.668 \end{bmatrix}$ 。

2. 列归一化：
   * 列 1 和 = $0.881 + 0.332 = 1.213 \implies [0.881/1.213, 0.332/1.213]^T \approx [0.726, 0.274]^T$
   * 列 2 和 = $0.119 + 0.668 = 0.787 \implies [0.119/0.787, 0.668/0.787]^T \approx [0.151, 0.849]^T$
   得到 $A^{(2)} = \begin{bmatrix} 0.726 & 0.151 \\ 0.274 & 0.849 \end{bmatrix}$ 。

经过几轮迭代后，矩阵最终收敛至双随机矩阵 $H_l$ （行列和均极度逼近 1，这里假设收敛值为）：

$$
H_l \approx \begin{bmatrix} 0.8 & 0.2 \\ 0.2 & 0.8 \end{bmatrix}
$$

此时，该矩阵的最大奇异值（谱范数）被严格限制在 $\|H_l\|_2 = 1.0$ 。

**第三步：多流混合计算 $X'_l = H_l X_l$**
我们再对相同的输入状态 $X_l = \begin{bmatrix} 1.0 & 2.0 \\ -1.0 & 0.0 \end{bmatrix}$ 进行混合计算：

$$
X'_l = \begin{bmatrix} 0.8 & 0.2 \\ 0.2 & 0.8 \end{bmatrix} \begin{bmatrix} 1.0 & 2.0 \\ -1.0 & 0.0 \end{bmatrix} = \begin{bmatrix} 0.6 & 1.6 \\ -0.6 & 0.4 \end{bmatrix}
$$

计算混合后两流状态的模长平方和：

$$
\|X'_l\|_F^2 = 0.6^2 + 1.6^2 + (-0.6)^2 + 0.4^2 = 3.44
$$

**结论**：由于双随机约束将谱范数限制在 1，混合后状态的总能量从 $6.0$ 变成了 $3.44$ （未发生膨胀，而是进行了稳定的特征加权平滑），验证了 mHC 极佳的数值稳定性。
